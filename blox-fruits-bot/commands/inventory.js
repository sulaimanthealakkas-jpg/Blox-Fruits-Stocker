...
function buildPage(items, page, target) {
  ...
  // Dominant rarity colour
  const top = items[0];
  const color = top ? RARITY_COLOR[top.rarity] ?? 0xFFA500 : 0xFFA500;

  const lines = slice.map((f, i) => {
    const num = page * PAGE_SIZE + i + 1;
    return `\`${String(num).padStart(2, '0')}\` ${f.emoji} **${f.name}** — ${f.rarity} ${f.type}`;
  });
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: '🍎 Total Fruits', value: `\`${items.length}\``, inline: true },
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(o =>
      o.setName('user').setDescription('User to check (default: you)')
    ),
  ...
  async execute(interaction) {
    ...
    collector.on('collect', async btn => {
      ...
      const freshItems = [...getInventory(interaction.guildId, target.id)].sort(
        (a, b) => RARITY_ORDER.indexOf(a.rarity) - RARITY_ORDER.indexOf(b.rarity)
      );
      const { embed: newEmbed, totalPages: newTotal } = buildPage(freshItems, page, target);
      await btn.update({ embeds: [newEmbed], components: [buildNav(page, newTotal)] });
    });
