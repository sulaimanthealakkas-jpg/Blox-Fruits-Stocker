  ...
  async execute(interaction) {
    ...
    await interaction.reply({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription('Filter by rarity with the buttons, then pick from the dropdown.')
          .setColor(RARITY_COLOR[rarity])
          .setFooter({ text: 'Times out in 60 seconds' }),
      ],
      components: [rarityButtons(rarity), selectRow(rarity)],
    ...
    collector.on('collect', async i => {
      ...
      if (i.customId === 'additem_pick') {
        const fruit = allFruits.find(f => f.name === i.values[0]);
        ...
        await i.update({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(`${getFruitEmoji(fruit.name)} **${fruit.name}** has been added to your inventory.`)
              .addFields(
